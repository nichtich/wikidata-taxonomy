...
function buildTaxonomy (root, results, env) {
  ...
  results[0].forEach((row) => {
    ...
    if (!items[qid]) {
      ...
      if (row.item.label && row.item.label !== qid) {
  ...
  if (results[1]) {
    results[1].forEach((row) => {
      ...
      const instanceId = row.instance.value

      const item = { notation: [instanceId] }
      if (row.instance.label && row.instance.label !== instanceId) {
        item.prefLabel = { [language]: row.instance.label }
      }
      if (row.instanceDescription) {
        item.scopeNote = { [language]: row.instanceDescription }
      }
      ...
      if (instances[classId]) {
        instances[classId].push(instanceId)
      } else {
        instances[classId] = [ instanceId ]
      }
    })
